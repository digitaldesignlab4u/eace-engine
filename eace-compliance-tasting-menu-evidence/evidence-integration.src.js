/* ═══════════════════════════════════════════════════
   ...
   demo/UI glue — all the actual evidence logic lives in evidence-core.js
   and evidence-mode.js, which know nothing about this page's DOM.
═══════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function renderEvidence(evidence, token) {
    ...
    panel.innerHTML =
      ...
      '<strong>It does not prove your organisation attested to this session, and its timestamp is not independently verifiable</strong> — see what each field does and does not claim in evidence-verifier.html.</p>' +
      ...
      '<p class="hint">Paste this into evidence-verifier.html to check the question-set hash, the score-vs-answers consistency, and the device signature.</p>' +
      '<div class="evidence-upgrade">' +
        ...
        '<p class="hint">Adding an organisation co-signature and a trusted timestamp genuinely requires a network call to something this browser does not control — that is a real change to this product\'s local-first, no-backend architecture, not a missing feature of this demo. See README.md\'s "Grade 2 requires leaving local-first" section for the exact trade-off and what deploying it would involve.</p>' +
      '</div>';
  ...
  document.addEventListener('eace:session-complete', function (evt) {
    ...
    if (typeof EvidenceMode === 'undefined') {
      console.error('Evidence Mode scripts did not load — evidence-core.js/evidence-mode.js missing?');
      return;
    }
